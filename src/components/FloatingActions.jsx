...
export default function FloatingActions() {
  ...
  const handleFormSubmit = (e) => {
    ...
    const err = validatePhone(formData.contactNumber);
    if (err) {
      setPhoneError(err);
      return;
    }
                ...
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 rounded-xl text-xs bg-sand-50 border ${phoneError ? 'border-red-500 ring-2 ring-red-200' : 'border-gray-200 focus:border-brand-cyan'} focus:outline-none focus:bg-white text-brand-slate font-medium transition-all`}
                    />
                    {phoneError && (
